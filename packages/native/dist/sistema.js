 ...
 */
export function DatePicker({ value, onChange, mode = "date", minimumDate, maximumDate, disabled, size, format, placeholder, icon = "calendar", style, testID, }) {
    ...
    const receber = React.useCallback((evento, data) => {
    ...
    const escolher = React.useCallback(async () => {
        if (source === "camera") {
            ...
            if (!atual.granted && atual.canAskAgain)
                resposta = await ImagePicker.requestCameraPermissionsAsync();
            if (!resposta.granted) {
