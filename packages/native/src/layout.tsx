...
const ALINHAR = {start: "flex-start", center: "center", end: "flex-end", baseline: "baseline"} as const;
const JUSTIFICAR = {start: "flex-start", center: "center", end: "flex-end", between: "space-between"} as const;
 ...
 */
export function Cluster({align, justify, wrap, style, ...rest}: ClusterProps) {
  const s = folha(useAureaTokens());
  return (
    <View
      style={[
...
export function Grid({minColumnWidth = 240, style, children, onLayout, ...rest}: GridProps) {
  ...
  const celula = React.useMemo(() => {
    if (largura == null || largura <= 0) {
      return {flexGrow: 0, flexShrink: 1, flexBasis: minColumnWidth, minWidth: minColumnWidth, maxWidth: "100%" as const};
    }
  ...
  return (
    <View style={[s.grid, style]} {...rest}
      onLayout={(e) => { setLargura(e.nativeEvent.layout.width - recuo); onLayout?.(e); }}>
      {React.Children.map(children, (filho) => {
        if (!React.isValidElement(filho)) return filho;
        ...
        const proprio = (filho.props as {style?: StyleProp<ViewStyle>}).style;
        const estica = React.cloneElement(
          filho as React.ReactElement<{style?: StyleProp<ViewStyle>}>,
          {style: [{flexGrow: 1}, proprio] as StyleProp<ViewStyle>});
        return <View style={celula}>{estica}</View>;
      })}
    </View>
  );
}
 ...
 * comportamento certo.
 */
export function Separator({orientation = "horizontal", label, style, ...rest}: SeparatorProps) {
  const t = useAureaTokens();
  const s = folha(t);

  if (label != null) {
    return (
      <View style={[s.separadorRotulado, style]} {...rest}>
        <View style={s.separadorPedaco} />
        {typeof label === "string"
  ...
  return (
    <View
      style={[s.separador, orientation === "vertical" ? s.separadorV : s.separadorH, style]}
      {...rest}
    />
  );
}
...
export function Card({variant, style, ...rest}: CardProps) {
  ...
  return variant === "brand"
    ? <SobreAMarca.Provider value={{tinta: t.color.primaryForeground, fundo: t.color.primary}}>{pele}</SobreAMarca.Provider>
    : pele;
}
