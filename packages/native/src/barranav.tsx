...
interface Medida {
  /** Altura da pílula + as duas margens + a folga do sistema. Zero na variante `edge`. */
  espaco: number;
  anotar: (n: number) => void;
}

const BarraCtx = React.createContext<Medida | null>(null);
...
export function BottomNavProvider({children}: {children?: React.ReactNode}) {
  ...
  const valor = React.useMemo<Medida>(() => ({espaco, anotar: setEspaco}), [espaco]);
  return <BarraCtx.Provider value={valor}>{children}</BarraCtx.Provider>;
}
 ...
 */
export function useAnotarBottomNav(): ((n: number) => void) | null {
