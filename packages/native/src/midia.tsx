...
function razao(v: number | string | undefined): number | undefined {
...
const fonteDaImagem = (s: AureaImageSource): ImageSourcePropType =>
...
export function Gallery({
  items, label, selected, onSelect, zoom, ratio = 1, minTileWidth = 128, style, testID,
}: GalleryProps) {
  ...
  return (
    <>
      <Grid
        ...
        accessibilityLabel={label ?? strings.galleryLabel}
        style={style}>
        {items.map((item) => {
          const temLegenda = item.caption != null;
          const miolo = (
            <>
              <Image source={item.source} alt={temLegenda ? "" : item.alt} ratio={ratio} />
              {temLegenda && (typeof item.caption === "string"
                ? <Text size="sm" numberOfLines={2}>{item.caption}</Text>
                : item.caption)}
            </>
          );
          if (!interativo) {
            return <View key={item.id} style={s.ladrilho}>{miolo}</View>;
          }
          return (
            <Pressable
              key={item.id}
              testID={testID ? `${testID}-${item.id}` : undefined}
              onPress={() => { onSelect?.(item.id); if (zoom) setAmpliado(item.id); }}
      ...
      {/* O `Dialog` fica montado e fechado quando não há foto ampliada — é o contrato dele, e o
