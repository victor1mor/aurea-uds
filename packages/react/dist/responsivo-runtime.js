...
import { ESCALA, ESCALA_CONTAINER, PONTOS_DESC, PONTOS_CONTAINER_DESC } from "./escala.js";
import { cx, valorBase } from "./pure.js";
...
const ContainerAurea = createContext(null);
...
export function ContainerScope({ children, className, ...props }) {
    ...
    return _jsx(ContainerAurea.Provider, { value: ref, children: _jsx("div", { ref: ref, className: cx("container-scope", className), ...props, children: children }) });
}
...
const interessadosVp = new Set();
let ouvindoVp = false;
const avisarTodos = () => { for (const f of interessadosVp)
    f(); };
function assinarViewport(avisar) {
    if (!temMatchMedia())
        return () => { };
    interessadosVp.add(avisar);
    if (!ouvindoVp) {
        for (const px of Object.values(ESCALA))
            mql(px).addEventListener("change", avisarTodos);
        ouvindoVp = true;
    }
...
function pontoDaViewport() {
    if (!temMatchMedia())
        return null;
    for (const p of PONTOS_DESC)
        if (mql(ESCALA[p]).matches)
            return p;
    return null;
}
...
let observer = null;
function observarElemento(el, avisar) {
    ...
    if (!observer) {
        observer = new ResizeObserver((entradas) => {
            for (const e of entradas) {
                ...
                for (const f of inscritos.get(e.target) ?? [])
                    f(largura);
            }
        });
    }
...
function pontoDaLargura(largura) {
    for (const p of PONTOS_CONTAINER_DESC)
        if (largura >= ESCALA_CONTAINER[p])
            return p;
    return null;
}
// ── a resolução ────────────────────────────────────────────────────────────────────────────────
...
function doMapa(mapa, alcancado, ordemDesc, base) {
    if (!alcancado)
        return base;
    let vendo = false;
...
export function useValorResponsivo(valor, padrao, ancora) {
    const base = (valorBase(valor) ?? padrao);
    ...
    const pontoVp = useSyncExternalStore(porViewport ? assinarViewport : semAssinatura, () => (porViewport ? pontoDaViewport() : null), () => null);
    ...
    return useMemo(() => {
        if (porViewport)
            return doMapa(porViewport, pontoVp, PONTOS_DESC, base);
        if (porContainer)
            return doMapa(porContainer, largura === null ? null : pontoDaLargura(largura), PONTOS_CONTAINER_DESC, base);
        return base;
